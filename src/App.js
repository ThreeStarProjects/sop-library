...
// ─────────────────────────────────────────────────────────────────────────────
// DEFAULT DEPARTMENTS
// Each department has a sheetId and a tabs array (exact tab names from Google Sheet)
// ─────────────────────────────────────────────────────────────────────────────
var DEFAULT_DEPARTMENTS = [
  {
    id: "design", name: "Design", emoji: "🎨",
    sheetId: "1ayUXpV37H0-JQkP-QQTQ3mjtcJ7p26jalYDadC6qpg8",
    tabs: "Client Requirement Collection,Floor Plan Development,Design Development & Client Ppt,BOQ & Quotation,Quotation Negotiation & Approval,Internal Handover to Production,Material Finalisation,Detailed Production Drawing"
  },
  {
    id: "individual", name: "Individual", emoji: "👤",
    sheetId: "1KJQQcOJCkcsFssCyluuKXGnPqtNeFXAeQ6rmAqEtDVg",
    tabs: "Senior Interior Designer,Store Incharge,Site Supervisor,Factory Head"
  },
  {
    id: "installation", name: "Installation", emoji: "🔧",
    sheetId: "1lDl4QYjfivYr4tw1y6XXX50soci0iA3CSlbPaDoeIsU",
    tabs: "Material Receiving & Verification,Installation Planning & Organisation,Furniture Installation,Quality Inspection,Handover"
  },
  {
    id: "production", name: "Production", emoji: "🏭",
    sheetId: "163_mQeXKI8tAaotEhHuNuwQGqFfha3PeuIuaqSVq0fc",
    tabs: "Planning & Allocation,Material Cutting Process,CNC Machine,Edge Banding Process,Assembly Process,Polishing & Finishing Process,Production Quality Inspection,Material Dispatch,Machine Maintenance,Factory Safety & Operations,Waste Management Process,Tool Management Process,Production Progress Tracking"
  },
  {
...
var SAMPLE_PROCESSES = [
  {
    name: "Sample — Add your Sheet tab names in Supervisor Panel",
    steps: [
      {
        stage: "Setup", step: 1,
        name: "Configure Your Department",
        input: "Your Google Sheet link",
        process: "Go to Supervisor Panel → Departments → Edit your department → Add tab names exactly as they appear in your Google Sheet (comma separated).",
        output: "Live SOP data",
        responsible: "Supervisor", reviewer: "", tat: "5 Min",
        escalation: "", tools: "", safety: "", referenceDoc: "",
        dos: "Copy tab names exactly from your Google Sheet bottom tabs.",
        donts: "Do not add extra spaces before or after tab names.",
        commonMistakes: "Spelling mistakes in tab names. Extra spaces.",
        whatIf: "If data still does not load, check the Sheet is set to Anyone with link Viewer.",
        qualityCheck: "Good: Live data shows with correct process names. Bad: Sample data still showing.",
      },
    ],
  },
];
...
// ─────────────────────────────────────────────────────────────────────────────
// GOOGLE SHEETS — fetch steps from a specific tab by name
// ─────────────────────────────────────────────────────────────────────────────
...
// Parse comma-separated tab names string
function parseTabs(tabsStr) {
  if (!tabsStr || tabsStr.trim() === "") return [];
  return tabsStr.split(",").map(function(t) { return t.trim(); }).filter(function(t) { return t !== ""; });
}
  ...
  return (
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.97)", zIndex:9999, display:"flex", flexDirection:"column", padding:16, overflowY:"auto" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        <span style={{ color:B.orange, fontWeight:800, fontSize:16 }}>🔍 Debug Log</span>
      ...
      {props.log.length === 0
        ...
        : props.log.map(function(line, i) {
          ...
          else if (line.indexOf("✓") !== -1 || line.indexOf("steps") !== -1) color = "#86efac";
...
function SupervisorPanel(props) {
  ...
  var [formTabs, setFormTabs]       = useState("");
  ...
  function clearForm() {
    setEditingDept(null);
    setFormName(""); setFormSheetId(""); setFormTabs(""); setFormEmoji("🏢"); setFormMsg("");
  }
    ...
    if (editingDept) {
      setDepartments(function(ds) {
        var updated = ds.map(function(d) {
          return d.id === editingDept.id
            ? { id:d.id, name:formName.trim(), emoji:formEmoji, sheetId:sheetId, tabs:formTabs.trim() }
            : d;
        });
        saveDepts(updated);
        return updated;
      });
      setFormMsg("✅ Updated! Click Refresh SOPs to reload data.");
    } else {
      var newDept = { id:makeId(), name:formName.trim(), emoji:formEmoji, sheetId:sheetId, tabs:formTabs.trim() };
      setDepartments(function(ds) {
        var updated = ds.concat([newDept]);
        saveDepts(updated);
        return updated;
      });
      setFormMsg("✅ Added! Click Refresh SOPs to load data.");
    }
      ...
      <div style={{ padding:"18px 14px" }}>
            ...
            {departments.map(function(dept) {
              ...
              return (
                <div key={dept.id} style={{ background:B.white, borderRadius:16, padding:16, marginBottom:12, border:"1px solid #e2e8f0" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:8 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ color:"#64748b", fontSize:12 }}>
                        {tabs.length === 0
                          ? "⚠️ No tab names added — edit to configure"
                          : d ? procCount + " processes · " + stepCount + " steps" : "Configured · click Refresh"}
                      </div>
                    </div>
                    <div style={{ background: d ? "#dcfce7" : tabs.length === 0 ? "#fef9c3" : "#fee2e2", color: d ? "#16a34a" : tabs.length === 0 ? "#854d0e" : "#dc2626", borderRadius:20, padding:"4px 12px", fontSize:12, fontWeight:700, flexShrink:0 }}>
                      {d ? "✓ Live" : tabs.length === 0 ? "⚠ Setup" : "✗ Sample"}
                    </div>
                  </div>
                  ...
                  {tabs.length > 0 && !d && (
                    <div style={{ background:"#f8fafc", borderRadius:10, padding:"7px 12px", fontSize:12, color:"#94a3b8" }}>
                      Tabs configured: {tabs.join(", ")}
                    </div>
                  )}
                </div>
              );
          ...
          <div>
            <div style={{ background:B.white, borderRadius:16, padding:16, marginBottom:16, border:"1px solid #e2e8f0" }}>
              ...
              <input value={formSheetId} onChange={function(e){ setFormSheetId(e.target.value); }}
                ...
                style={{ width:"100%", padding:"10px 14px", borderRadius:10, border:"2px solid #e2e8f0", fontSize:13, fontFamily:"monospace", outline:"none", marginBottom:12, boxSizing:"border-box" }} />

              <label style={{ color:"#374151", fontSize:13, fontWeight:700, display:"block", marginBottom:4 }}>
                Sheet Tab Names * <span style={{ color:"#94a3b8", fontWeight:400 }}>(comma separated, exactly as in Google Sheet)</span>
              </label>
              <textarea value={formTabs} onChange={function(e){ setFormTabs(e.target.value); }} rows={3}
                placeholder="Client Requirement Collection,Floor Plan Development,BOQ & Quotation"
                style={{ width:"100%", padding:"10px 14px", borderRadius:10, border:"2px solid #e2e8f0", fontSize:13, outline:"none", marginBottom:4, boxSizing:"border-box", resize:"vertical" }} />
              <div style={{ background:"#fff7ed", border:"1px solid #fed7aa", borderRadius:10, padding:"10px 14px", marginBottom:14 }}>
                <p style={{ color:"#92400e", fontSize:12, fontWeight:700, marginBottom:4 }}>📌 How to get tab names:</p>
                <p style={{ color:"#92400e", fontSize:12 }}>Open your Google Sheet → look at the tabs at the bottom → copy each name exactly → paste here separated by commas.</p>
              </div>

              {formMsg !== "" && (
                <p style={{ color: formMsg.indexOf("✅") !== -1 ? "#16a34a" : "#dc2626", fontWeight:700, marginBottom:10, fontSize:14 }}>{formMsg}</p>
              )}
            ...
            {departments.map(function(dept){
              var tabs = parseTabs(dept.tabs);
              return (
                <div key={dept.id} style={{ background:B.white, borderRadius:14, padding:"14px 16px", marginBottom:10, border:"1px solid #e2e8f0" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom: tabs.length > 0 ? 8 : 0 }}>
                    <span style={{ fontSize:26 }}>{dept.emoji}</span>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontWeight:800, fontSize:15, color:B.primaryBlue }}>{dept.name}</div>
                      <div style={{ color:"#94a3b8", fontSize:11 }}>{tabs.length > 0 ? tabs.length + " tabs configured" : "⚠️ No tabs — tap ✏️ to add"}</div>
                    </div>
                  ...
                  {tabs.length > 0 && (
                    <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:4 }}>
                      {tabs.map(function(t){
                        return <span key={t} style={{ background:"#eff6ff", color:B.primaryBlue, borderRadius:8, padding:"3px 10px", fontSize:11, fontWeight:600 }}>{t}</span>;
                      })}
                    </div>
                  )}
                </div>
        ...
        {tab === "columns" && (
          <div style={{ background:B.white, borderRadius:16, padding:16, border:"1px solid #e2e8f0" }}>
            ...
            {[
              ["STAGE","Group name e.g. 01 - Initial Client Engagement"],
              ...
              ["TAT","Time allowed e.g. 1 Day, 4 Hours"],
...
export default function App() {
  ...
  var loadFromSheets = useCallback(function() {
    ...
    depts.forEach(function(dept) {
      ...
      if (!sheetId) {
        addLog("  FAILED: No Sheet ID");
        results[dept.id] = null;
        checkDone();
        return;
      }

      if (tabs.length === 0) {
        addLog("  SKIPPED: No tab names configured — go to Supervisor > Depts > Edit to add tab names");
        results[dept.id] = null;
        checkDone();
        return;
      }

      addLog("  tabs configured: [" + tabs.join(", ") + "]");

      var tabResults = [];
      var tabsRemaining = tabs.length;

      function checkTabsDone() {
        tabsRemaining--;
        if (tabsRemaining === 0) {
          if (tabResults.length > 0) {
            results[dept.id] = { processes: tabResults };
            addLog("  ✓ " + dept.name + ": " + tabResults.length + " processes loaded");
          } else {
            results[dept.id] = null;
            addLog("  ✗ " + dept.name + ": no steps found — check tab names match exactly");
          }
          checkDone();
        }
      }
      ...
      tabs.forEach(function(tabName) {
        fetchTabSteps(sheetId, tabName).then(function(steps) {
          addLog("  \"" + tabName + "\" → " + steps.length + " steps");
          if (steps.length > 0) tabResults.push({ name: tabName, steps: steps });
          checkTabsDone();
        }).catch(function(e) {
          addLog("  \"" + tabName + "\" error: " + e.message);
          checkTabsDone();
        });
      });
    });
  ...
  useEffect(function() { loadFromSheets(); }, []);
  ...
  function goToDept(deptId) {
    ...
    if (!d || !d.processes || d.processes.length === 0) {
      setSelectedProc(SAMPLE_PROCESSES[0].name);
      setStepIdx(0);
      setScreen("steps");
      return;
    }
    if (d.processes.length === 1) {
      setSelectedProc(d.processes[0].name);
      setStepIdx(0);
      setScreen("steps");
    } else {
      setScreen("procs");
  ...
  if (screen === "pin") {
    return (
      <PinScreen lang={lang} pin={pin} setPin={setPin} err={pinErr} setErr={setPinErr}
  ...
  if (screen === "supervisor") {
    return (
      <SupervisorPanel departments={departments} setDepartments={setDepartments}
        fetchedData={fetchedData} loading={loading} log={log}
        onBack={function(){ setScreen("home"); }} onRefresh={loadFromSheets} />
    );
  }

  if (screen === "step" && step) {
    return (
      <StepView step={step} stepNum={stepIdx+1} total={steps.length}
  ...
  if (screen === "steps") {
    var currentSteps = procObj ? procObj.steps : SAMPLE_PROCESSES[0].steps;
    ...
    return (
      <div style={{ minHeight:"100vh", background:B.darkBg }}>
        <GradHeader onBack={function(){ setScreen(backFromSteps); }} lang={lang} setLang={setLang}>
          ...
          <p style={{ color:B.lightGray, fontSize:13, opacity:0.7 }}>{currentSteps.length} {tr("steps","चरण",lang)}</p>
        </GradHeader>
        <div style={{ padding:"16px 14px" }}>
          {currentSteps.map(function(s, i){
            return (
  ...
  if (screen === "procs") {
    var procList = deptData ? deptData.processes : SAMPLE_PROCESSES;
    return (
      <div style={{ minHeight:"100vh", background:B.darkBg }}>
              ...
              <button key={dept.id} onClick={function(){ goToDept(dept.id); }}
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ color:"#888", fontSize:13, marginTop:2 }}>
                    {d ? d.processes.length + " " + tr("processes","प्रक्रियाएं",lang)
                      : tabs.length === 0 ? "⚠️ Setup required"
                      : tr("Loading…","लोड हो रहा है…",lang)}
                  </div>
                </div>
                <span style={{ color:B.tealBlue, fontSize:22 }}>›</span>
              </button>
