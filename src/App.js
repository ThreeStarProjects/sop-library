...
// Fetch actual sheet tab names by scraping the HTML page
// This is more reliable than the gviz metadata endpoint
async function fetchTabNames(sheetId) {
  ...
  // The gviz response embeds sheet/tab names in a specific pattern
  // Pattern: {"label":"TAB NAME","id":N} inside the response
  var tabNames = [];
  var seen = {};

  // Try extracting from "sheets" array pattern first
  var sheetsMatch = text.match(/"sheets":\s*\[([^\]]+)\]/);
  if (sheetsMatch) {
    var sheetsText = sheetsMatch[1];
    var labelPattern = /"label"\s*:\s*"([^"]+)"/g;
    var m;
    while ((m = labelPattern.exec(sheetsText)) !== null) {
      var n = m[1];
      if (n && !seen[n]) { seen[n] = true; tabNames.push(n); }
    }
  }

  // Fallback: try the cols label approach — fetch first sheet and look for sheet names
  // in the actual gviz JSON structure
  if (tabNames.length === 0) {
    try {
      var json = parseGViz(text);
      // Some versions of gviz embed sheet names differently
      // Try reading from cols where label might be sheet names
      var cols = json && json.table && json.table.cols ? json.table.cols : [];
      cols.forEach(function(c) {
        if (c && c.label && c.label.trim() !== "" && !seen[c.label]) {
          seen[c.label] = true;
          tabNames.push(c.label.trim());
        }
      });
    } catch(e) {}
  }

  // Last resort: use rows approach (original method)
  if (tabNames.length === 0) {
    try {
      var json2 = parseGViz(text);
      var rows = json2 && json2.table && json2.table.rows ? json2.table.rows : [];
      rows.forEach(function(r) {
        var v = r && r.c && r.c[0] && r.c[0].v ? String(r.c[0].v).trim() : "";
        if (v && !seen[v]) { seen[v] = true; tabNames.push(v); }
      });
    } catch(e) {}
  }
