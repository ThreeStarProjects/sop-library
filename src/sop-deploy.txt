...
// ─────────────────────────────────────────────────────────────────────────────
const PIN = "1234";
const LS_DEPTS_KEY = "ssp_departments_v3";
    ...
    steps: [
      {
        ...
        commonMistakes: "Forgetting to ask about site ownership. Not recording client backup contact.",
        whatIf: "If client changes requirements mid-meeting, pause, document changes and get written confirmation.",
        qualityCheck: "Good: Signed notes with clear budget range. Bad: Verbal agreement only, no documentation.",
      },
      {
        ...
        donts: "Do not use informal language. Do not leave any section blank.",
        ...
        qualityCheck: "Good: All sections filled, client signed. Bad: Incomplete or missing signature.",
      },
    ],
  },
];
...
async function fetchTabNames(sheetId) {
  ...
  return rows
    .map(function(r) { return r && r.c && r.c[0] && r.c[0].v ? r.c[0].v : null; })
...
function loadDepts() {
  try {
    const saved = localStorage.getItem(LS_DEPTS_KEY);
...
function StepView(props) {
  ...
  for (var b = 0; b < total; b++) {
    bars.push(
      <div key={b} style={{ flex:1, height:5, borderRadius:4, background: b < stepNum-1 ? B.orange : b === stepNum-1 ? B.white : "rgba(255,255,255,0.2)" }} />
    );
  }
            ...
            {departments.map(function(dept){
              return (
                <div key={dept.id} style={{ background:B.white, borderRadius:14, padding:"14px 16px", marginBottom:10, border:"1px solid #e2e8f0", display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <button onClick={function(){ openEditForm(dept); }}
                  ...
                  <button onClick={function(){ deleteDept(dept.id); }}
...
export default function App() {
  ...
  var loadFromSheets = useCallback(function() {
    ...
    depts.forEach(function(dept) {
      ...
      if (!sheetId) {
        ...
        remaining--;
        if (remaining === 0) { setFetchedData(Object.assign({}, results)); setLoading(false); addLog("── All done ──"); }
        return;
      }
      fetchTabNames(sheetId).then(function(tabs) {
        addLog("  tabs: [" + tabs.join(", ") + "]");
        var tabResults = [];
        var tabsRemaining = tabs.length;
        if (tabsRemaining === 0) {
          addLog("  No tabs found");
          results[dept.id] = { processes: SAMPLE_PROCESSES };
          remaining--;
          if (remaining === 0) { setFetchedData(Object.assign({}, results)); setLoading(false); addLog("── All done ──"); }
          return;
        ...
        tabs.forEach(function(tab) {
          fetchTabSteps(sheetId, tab).then(function(steps) {
            ...
            if (tabsRemaining === 0) {
              ...
              addLog("  ✓ " + dept.name + ": " + tabResults.length + " processes");
              remaining--;
              if (remaining === 0) { setFetchedData(Object.assign({}, results)); setLoading(false); addLog("── All done ──"); }
            }
          }).catch(function(e) {
  ...
  if (screen === "steps") {
    var backScreen = deptData && deptData.processes && deptData.processes.length > 1 ? "procs" : "depts";
    return (
      <div style={{ minHeight:"100vh", background:B.darkBg }}>
        <GradHeader onBack={function(){ setScreen(backScreen); }} lang={lang} setLang={setLang}>
